# Discord Utility Bot
多機能な Discord Bot プロジェクトです。リマインダー、ロールセレクター、メッセージピン留め、Lumacoin による独自通貨経済、モデレーションなどの機能を備えています。音声関連機能（VoiceVox 連携など）は後続フェーズで実装します。
...
- **Lumacoin エコノミー:** `/lumacoin` コマンドで残高確認・送金・デイリーボーナス受取ができ、ミニゲームでベット・配当が可能です（残高はギルド単位で管理されます）。
...
- `npm run dev` — nodemon + Node を使って起動します（既定）。この場合 Node は `.js` を読み込みます。
- `npm run dev:bun` — Bun で直接起動します（`.ts` をネイティブに読み込めるため、TypeScript 実装が優先されます）。
- `npm run dev:node` — Node を `ts-node` の ESM ローダー付きで起動します（`.ts` を Node で読み込めるようにします）。
...
| `LOGGER_NAME` | ログの `service` フィールドに使用する名前。 | `LOGGER_NAME=shiro-bot-staging` |
| `LOG_FILE_PATH` | 指定時、本番環境でこのパスにファイル出力を追加。 | `LOG_FILE_PATH=/var/log/shiro.log` |
...
| `/lumacoin balance` / `give` / `add` / `pay` / `daily` | Lumacoin の残高確認・送金・管理者付与・消費・デイリーボーナス受取を行います（残高はサーバーごとに独立）。 |
