...
import { createReminder, deleteReminder, listPendingReminders } from '../database/repositories/reminderRepository.ts';
...
function buildTaskKey(reminder: { id: number }) {
  return `reminder:${reminder.id}`;
}

async function runReminder(client: any, reminder: any) {
  try {
    ...
    // node-cron handles subsequent schedules automatically.
  } catch (error) {
...
function scheduleReminder(client: any, reminder: any) {
...
export async function bootstrapReminders(client: any) {
...
export async function registerReminder(client: any, discordUser: any, payload: any) {
