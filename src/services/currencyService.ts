...
import type { CurrencyBalance, CurrencyTransaction, User as PrismaUser } from '@prisma/client';
...
function resolveGuildContext(guild: any) {
...
async function ensureGuildRecord(guild: any, transaction?: any) {
...
async function getOrCreateBalance(guildId: string, userId: number, transaction?: any): Promise<CurrencyBalance> {
  const balance = await findBalanceByGuildAndUser(guildId, userId, transaction);
  if (balance) return balance as CurrencyBalance;
  return upsertBalance(guildId, userId, transaction) as Promise<CurrencyBalance>;
}
...
async function adjustBalance(
  ...
  delta: number,
  type: string,
  { reason, metadata }: { reason?: string; metadata?: any } = {}
) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const userRecord = (await getOrCreateUser(discordUser, tx)) as PrismaUser;
...
export async function getBalance(guild: any, discordUser: any) {
  const guildContext = await ensureGuildRecord(guild);
  const userRecord = (await getOrCreateUser(discordUser)) as PrismaUser;
...
export async function credit(guild: any, discordUser: any, amount: number, options: any = {}) {
  assertPositiveAmount(amount);
  const { type = TRANSACTION_TYPES.EARN } = options;
  return adjustBalance(guild, discordUser, amount, type, options);
}

export async function debit(guild: any, discordUser: any, amount: number, options: any = {}) {
  assertPositiveAmount(amount);
...
export async function transfer(
  ...
  return prisma.$transaction(async (tx) => {
    const guildContext = await ensureGuildRecord(guild, tx);
    const sender = (await getOrCreateUser(fromUser, tx)) as PrismaUser;
    const recipient = (await getOrCreateUser(toUser, tx)) as PrismaUser;
...
export async function claimDaily(guild: any, discordUser: any) {
  const now = new Date();
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (lastClaim) {
      const elapsed = now.getTime() - new Date((lastClaim as CurrencyTransaction).createdAt).getTime();
      if (elapsed < DAILY_COOLDOWN_MS) {
        const retryAt = new Date((lastClaim as CurrencyTransaction).createdAt).getTime() + DAILY_COOLDOWN_MS;
...
export function getCooldownInfo(error: unknown) {
  if (error instanceof CurrencyError && (error as any).code === 'COOLDOWN_ACTIVE') {
    return (error as any).context;
  }
  return null;
}

export async function placeBet(guild: any, discordUser: any, amount: number, metadata?: any) {
...
export async function payoutWin(guild: any, discordUser: any, amount: number, metadata?: any) {
...
export async function adjustBalanceManually(guild: any, discordUser: any, amount: number, { reason, metadata }: { reason?: string; metadata?: any } = {}) {
  if (!Number.isFinite(amount) || amount === 0) {
