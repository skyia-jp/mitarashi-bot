...
function calculatePayout(session: any) {
...
export async function settleBlackjackSession(interaction: any, session: any) {
  ...
  if (session.state.result === BlackjackResult.PLAYER_WIN) {
    ...
    if (totalBet > 0) {
      await credit(interaction.guild, interaction.user, totalBet, {
        ...
        metadata: { game: 'blackjack', interactionId: interaction.id, sessionId: session.id }
      });
    }
    netChange = 0;
  }
