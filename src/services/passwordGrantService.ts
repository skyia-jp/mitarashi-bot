...
async function fetchRole(guild: any, roleId: string | null) {
  if (!roleId) return null;
  return guild.roles.cache.get(roleId) ?? (await guild.roles.fetch(roleId).catch(() => null));
}

async function ensureBotCanAssignRole(guild: any, role: any) {
...
export async function handlePasswordSubmission(guild: any, member: any, password: string) {
