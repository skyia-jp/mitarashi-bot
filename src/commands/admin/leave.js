...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName('guild_id')
        .setDescription('指定したサーバー ID から退出します')
        .setRequired(false)
    ),
  async execute(client, interaction) {
    ...
    const logger = buildInteractionLogger(
      interaction,
      { module: 'command:leave', target_guild_id: targetGuildId ?? undefined }
    );
    ...
    const initialMessage = leavingCurrentGuild
      ? 'サーバーから退出します。'
      : `指定されたサーバー (${targetGuild.name ?? '名称不明'} / ${targetGuild.id}) から退出します。`;
    ...
    try {
      ...
      await interaction.followUp({ content: '退出に失敗しました。後で再度お試しください。', ephemeral });
    }
  }
};
