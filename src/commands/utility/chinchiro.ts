...
function formatDelta(amount: number) {
  ...
  if (!Number.isFinite(n)) return '±0 Lumacoin';
  if (n > 0) return `+${intl.format(n)} Lumacoin`;
  if (n < 0) return `-${intl.format(Math.abs(n))} Lumacoin`;
  return '±0 Lumacoin';
}
...
function rollDice() {
  return [Math.ceil(Math.random() * 6), Math.ceil(Math.random() * 6), Math.ceil(Math.random() * 6)].sort((a, b) => a - b);
}
...
function evaluateHand(dice: number[]) {
  ...
  if (is456) {
    return { rank: 5, label: 'シゴロ (4-5-6)', kicker: 6 } as any;
  }

  if (isTriple) {
    return { rank: 4, label: `${a}のゾロ目`, kicker: a } as any;
  }

  if (is123) {
    return { rank: -1, label: 'ヒフミ (1-2-3)', kicker: 0 } as any;
  }

  if (a === b) {
    return { rank: 3, label: `${c}の目`, kicker: c } as any;
  }

  if (b === c) {
    return { rank: 3, label: `${a}の目`, kicker: a } as any;
  }

  return { rank: 0, label: '目なし', kicker: 0 } as any;
}

function determineResult(player: any, dealer: any) {
...
function diceToEmoji(dice: number[]) {
...
function simulateGame() {
  ...
  while (!outcome && safetyCounter < 5) {
    ...
    roundData = { playerDice, dealerDice, playerHand, dealerHand };

    history.push({ round: safetyCounter, data: roundData, result });
  ...
  return { history, outcome, finalRound: history.at(-1) } as any;
}

function buildEmbed(attempts: any[], outcome: string, winRate: number) {
  ...
  if (attempts.length > 1 || finalAttempt.history.length > 1) {
    embed.fields = attempts.map((attempt: any, index: number) => ({
...
export default {
  data: new SlashCommandBuilder().setName('chinchiro').setDescription('チンチロリンでボットと勝負！').addIntegerOption((option) => option.setName('bet').setDescription('賭ける Lumacoin の額 (任意)').setMinValue(1)),
  async execute(client: Client, interaction: ChatInputCommandInteraction) {
    ...
    try {
      if (betAmount > 0) {
        await placeBet(interaction.guild, interaction.user, betAmount, { game: 'chinchiro', interactionId: interaction.id } as any);
        betPlaced = true;
      }

      const bias = (await prepareGameBias(interaction, 'chinchiro')) as any;
      ...
      if (betPlaced) {
        if (game.outcome === 'player') {
          ...
          await payoutWin(interaction.guild, interaction.user, payout, { game: 'chinchiro', interactionId: interaction.id, originalBet: betAmount } as any);
          netChange = betAmount;
      ...
      if (betPlaced) {
        const { balance } = (await getBalance(interaction.guild, interaction.user)) as any;
      ...
      if (error instanceof CurrencyError) {
        ...
        if (err.code === 'INSUFFICIENT_FUNDS') {
          await interaction.editReply({ content: `💸 残高が不足しています。（必要: ${formatCoins(err.context?.required ?? betAmount)}）` });
          return;
        }
