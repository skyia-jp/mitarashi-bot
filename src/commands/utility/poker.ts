...
function formatEvaluationDetail(evaluation: any) {
  const labels = evaluation.strength.map((value: any) => valueToLabel(value));
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName('bet')
        .setDescription('賭ける Lumacoin の額 (任意)')
        .setMinValue(1)
    ),
  ...
  const balanceRes: any = await getBalance(interaction.guild, interaction.user);
  const remaining = balanceRes?.balance?.balance ?? (typeof balanceRes === 'number' ? balanceRes : balanceRes?.balance);
  const summary = `ベット: ${formatCoins(betAmount)} | 収支: ${formatDelta(netChange)} | 残高: ${formatCoins(remaining)}`;
      ...
      if (error instanceof CurrencyError) {
        if (error.code === 'INSUFFICIENT_FUNDS') {
          const required = (error as any).context?.required ?? betAmount;
          await interaction.editReply({
            content: `💸 残高が不足しています。（必要: ${formatCoins(required)}）`
          });
          return;
        }
