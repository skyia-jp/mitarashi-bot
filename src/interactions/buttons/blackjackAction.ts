...
function buildStatusMessage(action: string, session: any) {
...
export default {
  ...
  const session = getBlackjackSession(sessionId);
    ...
    const embedOpts: { revealDealer?: boolean; statusMessage?: string | null | undefined; balanceInfo?: { currentBalance?: number } | null } = {
      revealDealer: session.state.finished,
      statusMessage: buildStatusMessage(action, session),
      balanceInfo: settlement?.balanceInfo
    };
    const embed = buildBlackjackEmbed(session, embedOpts);
